...
|---|---:|---:|---:|---:|---:|
| T-Learner | 0.16% | 4.97% | 0.70% | 18.0 | Yes |
| S-Learner | 1.10% | 19.57% | 0.01% | 0.2 | No |
...
The top decile contains the customers predicted to be most persuadable by the email. For the selected model, the observed uplift in decile 1 was 0.16%. This is directionally useful, but it should not be the only selection metric because rare conversions make small slices of the test set volatile. The full dataset observed average treatment effect was 0.68%.

## Targeting Top 30% Uplift Users

Targeting the top 30% uplift-ranked users reaches 2,557 customers in the test set. The observed incremental conversion rate inside that targeted group is 0.70%, which corresponds to an estimated 18.0 incremental conversions.
